...
for distancia_sources in distances:
    ...
    obs = Observer()
    obs.add(Observer1D())
    output = TextOutput(f'testeFR/eventosTestFR/eventos_G/events_{k}FR.txt', Output.Event1D)
    ...
    # Carregar os eventos
    d = np.genfromtxt(f'testeFR/eventosTestFR/eventos_G/events_{k}FR.txt', names=True)
...
with open('testeFR/eventosTestFR/eventos_G/energia_mediaFR_p.txt', 'w') as f:
